      ...
      "in": {
        "$let": {
          "vars": { 
            ...
            "currentObjectChildren": getNestedChildrenOfSubdoc(getField("$$currentObject","subdoc")),
            "currentObjectIdx": getField("$$currentObject","index"),
            "currentSubdocPath": getField("$$currentObject","subdocpath"),
            "newDepthNumber": {"$add": [getField("$$currentObject","depth"), 1]},
          },
          "in": {
...
function buildArrayOfSchemaMetadataFields(object, currentResultPosition) {
  return [
    ...
    [{"k": "depth", "v": getField(object,"depth")}],
    [{"k": "index", "v": getField(object,"index")}],
    [{"k": "subdocpath", "v": getField(object,"subdocpath")}],                  
    
    // Capture the schema for each field for this new object with its name, value and type
    [{"k": "schema", "v": {
      "$map": {
        "input": {"$objectToArray": getField(object,"subdoc")},
        "as": "field",
        "in": {
          "fieldname": "$$field.k",
          "fieldvalue": {"$switch": {
          ...
          "in": {              
            "$concatArrays": [                            
              ...
              [{
                ...
                "subdocpath": {"$concat": [currentSubdocPath, "$$subdocPathSeperator", getField("$$childObject","key")]},
                "subdoc": getField("$$childObject","value"),                    
              }],
            ]                
          }
        }                                
